...
Instalación limpia: `001_baseline_v1.sql` → `002_cierre_fuentes_excel.sql` → `003_reporting_fuentes_excel.sql` → `004_seguimiento_funcional_excel.sql` → `005_reporting_cierre_excel.sql` → `006_ajustes_reporting_post_auditoria.sql`.
...
La siguiente migración es 006. El Modelo Excel V1 queda funcionalmente congelado en 005; 006+ sólo se justifica por requerimientos nuevos o defectos reales, no por campos ya presentes en las fuentes auditadas. No se editan migraciones ya publicadas; se agrega una nueva.
