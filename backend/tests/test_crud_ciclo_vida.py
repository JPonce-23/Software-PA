...
class TestCRUDTramos:
    ...
    def test_crear_tramo(self, client, admin_session, cleanup, seed_proyecto):
        uid = _uid()
        payload = {
    ...
    def test_leer_tramo_por_id(self, client, admin_session):
        res = client.get(f"/api/tramos/{_ids['tramo']}", headers=admin_session)
        assert res.status_code == 200
        assert res.json()["id_tramo"] == _ids["tramo"]

    def test_listar_tramos_incluye_creado(self, client, admin_session):
        res = client.get("/api/tramos", headers=admin_session)
        assert res.status_code == 200
        ids = [t["id_tramo"] for t in res.json()]
        assert _ids["tramo"] in ids

    def test_actualizar_nombre_tramo(self, client, admin_session):
        res = client.put(
    ...
    def test_no_permite_baja_de_proyecto_con_tramos_activos(
        self, client, admin_session, seed_proyecto
    ):
        res = client.delete(
    ...
    def test_tramo_inexistente_devuelve_404(self, client, admin_session):
        res = client.get("/api/tramos/999999", headers=admin_session)
...
class TestCRUDParcelas:
    ...
    def test_crear_parcela(self, client, admin_session, cleanup, seed_nucleo):
        uid = _uid()
        payload = {
    ...
    def test_leer_parcela(self, client, admin_session):
        res = client.get(f"/api/parcelas/{_ids['parcela']}", headers=admin_session)
        assert res.status_code == 200

    def test_actualizar_parcela(self, client, admin_session):
        res = client.put(
    ...
    def test_listar_parcelas_por_nucleo(self, client, admin_session, seed_nucleo):
        res = client.get(
...
class TestCRUDAfectaciones:
    ...
    def test_crear_afectacion_colectiva(
        self, client, admin_session, cleanup, seed_nucleo, seed_tramo_nucleo
    ):
        payload = {
            "id_nucleo": seed_nucleo["id_nucleo"],
    ...
    def test_crear_afectacion_individual(
        self, client, admin_session, cleanup, seed_nucleo, seed_tramo_nucleo, seed_parcela
    ):
        payload = {
            "id_nucleo": seed_nucleo["id_nucleo"],
            "id_tramo_nucleo": seed_tramo_nucleo["id_tramo_nucleo"],
    ...
    def test_listar_afectaciones_por_tramo_nucleo(
        self, client, admin_session, seed_tramo_nucleo
    ):
        tn_id = seed_tramo_nucleo["id_tramo_nucleo"]
        res = client.get(f"/api/afectaciones?id_tramo_nucleo={tn_id}", headers=admin_session)
        assert res.status_code == 200
        assert isinstance(res.json(), list)

    def test_actualizar_superficie(self, client, admin_session):
        res = client.put(
...
class TestCRUDAsambleas:
    ...
    def test_actualizar_resultado_anuencia(self, client, admin_session):
        res = client.put(
    ...
    def test_leer_asamblea_por_id(self, client, admin_session):
        res = client.get(f"/api/asambleas/{_ids['asamblea']}", headers=admin_session)
        assert res.status_code == 200
...
class TestCRUDConvenios:
    ...
    def test_crear_convenio_colectivo_cop(
        self, client, admin_session, cleanup,
        ...
        res = client.post("/api/convenios", json=payload, headers=admin_session)
        assert res.status_code == 201
        data = res.json()
    ...
    def test_actualizar_fecha_firma(self, client, admin_session):
        res = client.put(
    ...
    def test_listar_convenios_filtrados(self, client, admin_session, seed_tramo_nucleo):
        tn_id = seed_tramo_nucleo["id_tramo_nucleo"]
        res = client.get(f"/api/convenios?id_tramo_nucleo={tn_id}", headers=admin_session)
        assert res.status_code == 200
...
class TestCRUDOrvs:
    ...
    def test_crear_orv(self, client, admin_session, cleanup, seed_nucleo):
        payload = {
            "id_nucleo": seed_nucleo["id_nucleo"],
        ...
        res = client.post("/api/orvs", json=payload, headers=admin_session)
        assert res.status_code == 201
        data = res.json()
    ...
    def test_actualizar_orv(self, client, admin_session):
        res = client.put(
            f"/api/orvs/{_ids['orv']}",
...
class TestCRUDPadrones:
    ...
    def test_crear_padron(self, client, admin_session, cleanup, seed_nucleo):
        payload = {
            "id_nucleo": seed_nucleo["id_nucleo"],
            "fecha_padron": "2025-12-01",
            "numero_ejidatarios_comuneros": 150,
        }
        res = client.post("/api/padrones", json=payload, headers=admin_session)
        assert res.status_code == 201
...
class TestCRUDActividadesCampo:
    ...
    def test_crear_actividad(self, client, admin_session, cleanup, seed_tramo_nucleo):
        payload = {
            "id_tramo_nucleo": seed_tramo_nucleo["id_tramo_nucleo"],
        ...
        res = client.post("/api/actividades-campo", json=payload, headers=admin_session)
        assert res.status_code == 201
...
class TestCRUDDocumentacion:
    ...
    def test_crear_documento(self, client, admin_session, cleanup, seed_nucleo):
        payload = {
        ...
        res = client.post("/api/documentacion", json=payload, headers=admin_session)
        assert res.status_code == 201
...
class TestCRUDAlertas:
    ...
    def test_crear_alerta(self, client, admin_session, cleanup, seed_nucleo):
        payload = {
        ...
        res = client.post("/api/alertas", json=payload, headers=admin_session)
        assert res.status_code == 201
        data = res.json()
    ...
    def test_marcar_alerta_leida(self, client, admin_session):
        res = client.post(
