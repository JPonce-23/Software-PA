-- Credential security events and suppression of password-only audit snapshots.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE version = '009'
      AND checksum_sha256 = 'ef85729089cc0b5d3905169f87abd343a34473f7cb97717ff98b710b49f991bc'
  ) THEN
    RAISE EXCEPTION '010 requiere la migración 009 exacta';
  END IF;
  IF EXISTS (SELECT 1 FROM schema_migrations WHERE version = '010') THEN
    RAISE EXCEPTION '010 ya se encuentra registrada';
  END IF;
END $$;

SELECT pg_advisory_xact_lock(hashtextextended('software-pa:010:credenciales', 0));

ALTER TABLE evento_acceso DROP CONSTRAINT chk_auth_evento_tipo;
ALTER TABLE evento_acceso ADD CONSTRAINT chk_auth_evento_tipo CHECK (
  tipo_evento IN (
    'login_exitoso', 'login_fallido', 'cuenta_bloqueada', 'logout',
    'sesion_expirada', 'sesion_revocada', 'desbloqueo', 'cambio_correo',
    'cambio_contrasena', 'restablecimiento_contrasena'
  )
);

ALTER TABLE evento_acceso DROP CONSTRAINT chk_auth_evento_motivo;
ALTER TABLE evento_acceso ADD CONSTRAINT chk_auth_evento_motivo CHECK (
  motivo_codigo IN (
    'inicio_sesion', 'credenciales_invalidas', 'usuario_inactivo',
    'bloqueo_vigente', 'quinto_fallo', 'cierre_usuario', 'cierre_total',
    'revocacion_admin', 'expiracion_inactividad', 'expiracion_absoluta',
    'desbloqueo_admin', 'desbloqueo_recuperacion', 'cambio_correo_admin',
    'cambio_contrasena_usuario', 'restablecimiento_contrasena_admin'
  )
);
